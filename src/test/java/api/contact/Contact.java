...
public class Contact extends ApiBase {
    ...
    public ContactDto dataForEditContact(int id){ // зависимость ламбок для того, чтобы сп класс как сеттегр геттер и давать ему аннотации для работы с параментрами
        dto = new ContactDto();
        dto.setId(id);
